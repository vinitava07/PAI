...
class BaseALNClassifier:
    ...
    def _add_confusion_matrix_stats(self, ax, conf_matrix):
        """Adiciona estatísticas resumidas na matriz de confusão"""
        # Calcula métricas gerais
        total = np.sum(conf_matrix)
        accuracy = np.trace(conf_matrix) / total
        
        # Adiciona texto com estatísticas
        stats_text = f'Acurácia Geral: {accuracy:.3f}\nTotal de Amostras: {total}'
        ax.text(0.02, 0.98, stats_text, transform=ax.transAxes, 
               verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.8),
               fontsize=10)

    def plot_confusion_matrix(self, conf_matrix, normalize=False, title_suffix=""):
        ...
        if normalize:
            ...
            fmt = '.2%'
            title = f'Matriz de Confusão Normalizada - XGBoost ALN{title_suffix}'
            cbar_label = 'Proporção'
        else:
            conf_matrix_display = conf_matrix
            fmt = 'd'
            title = f'Matriz de Confusão - XGBoost ALN{title_suffix}'
            cbar_label = 'Contagem'
        ...
        for i in range(conf_matrix.shape[0]):
            for j in range(conf_matrix.shape[1]):
                if normalize:
                    text_val = f'{conf_matrix_norm[i, j]:.1%}\n({conf_matrix[i, j]})'
                else:
                    text_val = f'{conf_matrix[i, j]}'
                
                ax.text(j, i, text_val,
                       ha="center", va="center",
                       color="white" if conf_matrix_display[i, j] > thresh else "black",
                       fontsize=12, fontweight='bold')
        
        # Adiciona estatísticas adicionais
        self._add_confusion_matrix_stats(ax, conf_matrix)
        
        plt.tight_layout()
        plt.show()
        
        # Imprime métricas detalhadas
        self._print_detailed_metrics(conf_matrix)
...
def evaluate_on_test(model, X_test, y_test, classifier=None):
    """Avalia modelo no conjunto de teste com visualizações"""
    try:
        ...
        # Plota matriz de confusão se classifier fornecido
        if classifier is not None:
            print("\nGerando visualizações...")
            
            # Matriz de confusão padrão
            classifier.plot_confusion_matrix(conf_matrix, normalize=False, title_suffix=" (Teste)")
            
            # Matriz de confusão normalizada
            classifier.plot_confusion_matrix(conf_matrix, normalize=True, title_suffix=" (Teste - Normalizada)")
...
if __name__ == "__main__":
    try:
        ...
        input_size = 256  # Reduzido de 256 para 224
        ...
        # Avalia no teste
        y_pred_test, conf_matrix_test = evaluate_on_test(model, X_test, y_test, classifier)
